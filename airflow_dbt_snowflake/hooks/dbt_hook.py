...
class DbtCliHook(BaseHook):
    ...
    :type env: dict
    :param profiles_dir: If set, passed as `--profiles-dir` argument
        to the `dbt` command
    :type profiles_dir: str
    :param target: If set, passed as `--target` argument to `dbt` command
    :type dir: str
    :param dir: The directory to run the CLI in
    :type vars: str
    :param vars: If set, passed as the `--vars` argument to `dbt` command
    :type vars: dict
    :param full_refresh: If `True`, will fully-refresh incremental models.
    :type full_refresh: bool
    :param models: If set, passed as `--models` argument to `dbt` command
    :type models: str
    :param warn_error: If `True`, treat warnings as errors.
    :type warn_error: bool
    :param exclude: If set, passed as `--exclude` argument to `dbt` command
    :type exclude: str
    :param select: If set, passed as `--select` argument to `dbt` command
    :type select: str
    :param selector: If set, passed as `--selector` argument to `dbt` command
    :type selector: str
    :param dbt_bin: `dbt` CLI. Defaults to `dbt`, so assumes it's on your `PATH`
    :type dbt_bin: str
    ...
    def _dump_vars(self):
        # The dbt `vars` parameter is defined using YAML. Unfortunately the
        # standard YAML library for Python isn't very good
        # and I couldn't find an easy way to have it formatted correctly.
        # However, as YAML is a super-set of JSON, this works just fine.
    ...
    def run_cli(self, *command):
        ...
        sp = subprocess.Popen(
            ...
            stderr=subprocess.STDOUT,  # this makes errors also go to stdout
            cwd=self.dir,
            close_fds=True,
        )
        self.sp = sp
        ...
        if sp.returncode:
            raise AirflowException(f"dbt command failed. Output:\n{output}")
